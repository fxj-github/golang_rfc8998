...
DATA sm2const0<>+0x00(SB)/8, $0xffffffff00000000
DATA sm2const1<>+0x00(SB)/8, $0xfffffffeffffffff
...
DATA sm2one<>+0x00(SB)/8, $0x0000000000000001
DATA sm2one<>+0x08(SB)/8, $0x00000000ffffffff
DATA sm2one<>+0x10(SB)/8, $0x0000000000000000
DATA sm2one<>+0x18(SB)/8, $0x0000000100000000
GLOBL sm2const0<>(SB), 8, $8
GLOBL sm2const1<>(SB), 8, $8
GLOBL sm2ordK0<>(SB), 8, $8
GLOBL sm2ord<>(SB), 8, $32
GLOBL sm2one<>(SB), 8, $32

/* ---------------------------------------*/
// func sm2OrdLittleToBig(res *[32]byte, in *sm2OrdElement)
TEXT ·sm2OrdLittleToBig(SB),NOSPLIT,$0
	JMP ·sm2BigToLittle(SB)
/* ---------------------------------------*/
// func sm2OrdBigToLittle(res *sm2OrdElement, in *[32]byte)
TEXT ·sm2OrdBigToLittle(SB),NOSPLIT,$0
	JMP ·sm2BigToLittle(SB)
/* ---------------------------------------*/
// func sm2LittleToBig(res *[32]byte, in *sm2Element)
TEXT ·sm2LittleToBig(SB),NOSPLIT,$0
	JMP ·sm2BigToLittle(SB)
/* ---------------------------------------*/
// func sm2BigToLittle(res *sm2Element, in *[32]byte)
TEXT ·sm2BigToLittle(SB),NOSPLIT,$0
	...
	RET
/* ---------------------------------------*/
// func sm2MovCond(res, a, b *P256Point, cond int)
TEXT ·sm2MovCond(SB),NOSPLIT,$0
	MOVQ res+0(FP), res_ptr
	MOVQ a+8(FP), x_ptr
	MOVQ b+16(FP), y_ptr
	MOVQ cond+24(FP), X12
...
TEXT ·sm2NegCond(SB),NOSPLIT,$0
	MOVQ val+0(FP), res_ptr
	MOVQ cond+8(FP), t0
	// acc = poly
	MOVQ $-1, acc0
	MOVQ sm2const0<>(SB), acc1
	MOVQ $-1, acc2
	MOVQ sm2const1<>(SB), acc3
...
sqrLoop:
	...
	// Subtract sm2
	SUBQ $-1, acc0
	SBBQ sm2const0<>(SB) ,acc1
	SBBQ $-1, acc2
	SBBQ sm2const1<>(SB), acc3
	SBBQ $0, t0
...
TEXT ·sm2Mul(SB),NOSPLIT,$0
	...
	// Subtract sm2
	SUBQ $-1, acc4
	SBBQ sm2const0<>(SB) ,acc5
	SBBQ $-1, acc0
	SBBQ sm2const1<>(SB), acc1
	SBBQ $0, acc2
...
TEXT ·sm2FromMont(SB),NOSPLIT,$0
	...
	RET
/* ---------------------------------------*/
// func sm2Select(res *P256Point, table *sm2Table, idx int)
TEXT ·sm2Select(SB),NOSPLIT,$0
	MOVQ idx+16(FP),AX
	MOVQ table+8(FP),DI
	MOVQ res+0(FP),DX
...
TEXT ·sm2SelectAffine(SB),NOSPLIT,$0
	MOVQ idx+16(FP),AX
	MOVQ table+8(FP),DI
	MOVQ res+0(FP),DX
...
loop_select_base:
	...
	RET
/* ---------------------------------------*/
// func sm2OrdMul(res, in1, in2 *sm2OrdElement)
TEXT ·sm2OrdMul(SB),NOSPLIT,$0
	MOVQ res+0(FP), res_ptr
	MOVQ in1+8(FP), x_ptr
	MOVQ in2+16(FP), y_ptr
	// x * y[0]
	...
	RET
/* ---------------------------------------*/
// func sm2OrdSqr(res, in *sm2OrdElement, n int)
TEXT ·sm2OrdSqr(SB),NOSPLIT,$0
	MOVQ res+0(FP), res_ptr
	MOVQ in+8(FP), x_ptr
	MOVQ n+16(FP), BX
...
TEXT sm2SubInternal(SB),NOSPLIT,$0
	...
	ADDQ $-1, acc4
	ADCQ sm2const0<>(SB), acc5
	ADCQ $-1, acc6
	ADCQ sm2const1<>(SB), acc7
	ANDQ $1, mul0
	...
	RET
/* ---------------------------------------*/
TEXT sm2MulInternal(SB),NOSPLIT,$8
	MOVQ acc4, mul0
	MULQ t0
	MOVQ mul0, acc0
	MOVQ mul1, acc1
	...
	RET
/* ---------------------------------------*/
TEXT sm2SqrInternal(SB),NOSPLIT,$8
...
#define sm2MulBy2Inline\
	...
	MOVQ acc7, t3;\
	SUBQ $-1, t0;\
	SBBQ sm2const0<>(SB), t1;\
	SBBQ $-1, t2;\
	SBBQ sm2const1<>(SB), t3;\
	SBBQ $0, mul0;\
	CMOVQCS acc4, t0;\
	CMOVQCS acc5, t1;\
	CMOVQCS acc6, t2;\
...
// func sm2PointAddAffineAsm(res, in1 *P256Point, in2 *sm2AffinePoint, sign, sel, zero int)
TEXT ·sm2PointAddAffineAsm(SB),0,$512-48
	...
	MOVQ acc2, t2
	MOVQ acc3, t3
	// Add in case the operand was > sm2
	ADDQ $-1, acc0
	ADCQ sm2const0<>(SB), acc1
	ADCQ $-1, acc2
	ADCQ sm2const1<>(SB), acc3
	ADCQ $0, mul0
	CMOVQNE t0, acc0
	CMOVQNE t1, acc1
	CMOVQNE t2, acc2
	...
	sm2MulBy2Inline			// u1 * hˆ2 * 2, inline
	LDacc (rsqr)
	...
	MOVOU y2in(16*0), X11
	MOVOU y2in(16*1), X12
	MOVOU sm2one<>+0x00(SB), X13
	MOVOU sm2one<>+0x10(SB), X14
...
TEXT sm2IsZero(SB),NOSPLIT,$0
	...
	XORQ $-1, acc4
	XORQ sm2const0<>(SB), acc5
	XORQ $-1, acc6
	XORQ sm2const1<>(SB), acc7
	ORQ acc5, acc4
	ORQ acc6, acc4
	ORQ acc7, acc4
...
//func sm2PointAddAsm(res, in1, in2 *P256Point) int
TEXT ·sm2PointAddAsm(SB),0,$680-32
...
//func sm2PointDoubleAsm(res, in *P256Point)
TEXT ·sm2PointDoubleAsm(SB),NOSPLIT,$256-16
	// Move input to stack in order to free registers
	MOVQ res+0(FP), AX
	MOVQ in+8(FP), BX
